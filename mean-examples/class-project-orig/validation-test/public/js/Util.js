...
Util.formValidation = function(formId, obj){
    ...
    label = Util.getEl('#' + formId + ' label');
    i = 0;
    while(i < label.length){
        if(label[i].lastChild.nodeName.toLowerCase() === "span"){
            label[i].removeChild(label[i].lastChild);
        }
        i++;
    }
    i = 0;
    ...
    while(i < label.length){
        j = 0;
        while(j < obj.length){
            if(label[i].getAttribute('for') === obj[j].for && obj[j].status === 'error'){
                label[i].innerHTML += '<span> ' +  obj[j].msg + '</span>';
                break;
            }
            j++;
        }
